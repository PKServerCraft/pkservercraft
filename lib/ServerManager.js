...
function createDroplet(server) {
    var deferred = Q.defer();
    
    api.dropletsCreateNewDroplet(server.dns, "nyc3", "4gb", server.slug, {}, function (error, images) {
        if (error) {
            deferred.reject(error);
        } else {
            deferred.resolve();
...
exports.createServer = function (slug) {
    var deferred = Q.defer();
    
    exports.findServer(slug).then(function (server) {
        if (server.droplet.status === "inactive") {
            createDroplet(server).then(function () {
